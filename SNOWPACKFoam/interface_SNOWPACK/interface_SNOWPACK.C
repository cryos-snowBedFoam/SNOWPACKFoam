...
addSpecialKeyToCfg::addSpecialKeyToCfg()
					: cfg(cfgfile)
{
	addSpecialKeys(cfg);
}
	...
	if (!avgsum_time_series) {
		...
		if (mass_balance) {
			...
			if (!massBalanceCheck(currentSector, surfFluxes, tot_mass_in))
				if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg+", Mdata.date, "Mass error during initial check!");
		}
	}
                   ...
                   double &wind_scaling_factor, double &time_count_deltaHS)
{
	...
	if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Finished initializing station %s", vecStationIDs[i_stn].c_str());
	...
	for (size_t sector=slope.first; sector<slope.nSlopes; sector++) {
		if (vecSSdata[sector].profileDate != vecSSdata[slope.mainStation].profileDate) {
			if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "err", mio::Date(),
...
void writeForcing(Date d1, const Date& d2, const double& Tstep, IOManager &io)
{
	...
	if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg",  mio::Date(), "Reading and writing out forcing data...");
	...
	if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg",  mio::Date(), "Forcing data written out");
}

void printStartInfo(const SnowpackConfig& cfg) //, const std::string& name) // Jafari commented
{
	const bool useSoilLayers = cfg.get("SNP_SOIL", "Snowpack");
	if (useSoilLayers) {
		bool soil_flux = false;
		cfg.getValue("SOIL_FLUX", "Snowpack", soil_flux);
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg",  mio::Date(), "Start SNOWPACK w/ soil layers in %s mode", mode.c_str());
	} else {
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg",  mio::Date(), "Start SNOWPACK in %s mode", mode.c_str());
	}

	const std::string variant = cfg.get("VARIANT", "SnowpackAdvanced");
	if (variant != "DEFAULT") {
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg",  mio::Date(), "Variant is '%s'", variant.c_str());
	}
	//prn_msg(__FILE__, __LINE__, "msg-", mio::Date(),
	//        "%s compiled on %s at %s", name.c_str(), __DATE__, __TIME__); // Jafari commented

	if (mode != "OPERATIONAL") {
		const std::string experiment = cfg.get("EXPERIMENT", "Output");
		const std::string outpath = cfg.get("METEOPATH", "Output");
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Experiment : %s", experiment.c_str());
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Output dir : %s", outpath.c_str());
	}
}
...
SnowpackInterface::SnowpackInterface()
                   : addSpecialKeyToCfg(),io(cfg),snowpackio(cfg),Mdata(cfg),
                    surfFluxes(),sn_Bdata(),slope(cfg),cumsum(slope.nSlopes),
                    current_date(dateBegin), meteo(cfg)
{
	init_sn();
}

int SnowpackInterface::init_sn() 
{
...
//#endif
	...
	std::string begin_date_str = cfg.get("BEGINDATE", "Input"); 
	std::string end_date_str = cfg.get("ENDDATE", "Input"); 
	//parseCmdLine(argc, argv, begin_date_str, end_date_str); // Jafari deleted
	//SnowpackConfig cfg(cfgfile); // Jafari commented, we already have it in coupling interface class
	//addSpecialKeys(cfg); // Jafari commented, we already added special keys to cfg by constructing SnowpackInterface and then its parrent...  
	//Meteo meteo(cfg); // Jafari commented, we need to declare it in the class to be used not only in this function but also the others... 

	//mio::Timer meteoRead_timer; // Jafari commented, we already have it in coupling interface class
	//mio::Timer run_timer; // Jafari commented, we already have it in coupling interface class
	run_timer.start();
	nowSRT = time(NULL);// Jafari modified, we already declared it in coupling interface class
	//MainControl mn_ctrl; //Time step control parameters // Jafari commented, we already have it in coupling interface class

	i_time_zone = cfg.get("TIME_ZONE", "Input"); //get user provided input time_zone // Jafari modified, we already declared it in coupling interface class
	...
	const bool useSoilLayers_ = cfg.get("SNP_SOIL", "Snowpack"); useSoilLayers=useSoilLayers_;
	...
	backup_days_between = 400.; // Jafari modified, we already declared it in coupling interface class
	...
	first_backup = 0.; // Jafari modified, we already declared it in coupling interface class
	cfg.getValue("FIRST_BACKUP", "Output", first_backup, mio::IOUtils::nothrow);
	label_snow = true;	// Initialize to true to be compliant with legacy SNOWPACK, // Jafari modified, we already declared it in coupling interface class
	cfg.getValue("LABEL_SNOW", "Output", label_snow, mio::IOUtils::nothrow);

	const bool allow_inflate_ = cfg.get("INFLATE_ALLOW", "Snowpack", (mode == "OPERATIONAL")); allow_inflate=allow_inflate_;
	const bool prn_check_ = cfg.get("INFLATE_INFO", "Snowpack", false); prn_check=prn_check_;
	const bool grooming_ = cfg.get("SNOW_GROOMING", "TechSnow");	grooming=grooming_;
	const bool classify_profile_ = cfg.get("CLASSIFY_PROFILE", "Output"); classify_profile=classify_profile_;
	const bool profwrite_ = cfg.get("PROF_WRITE", "Output"); profwrite=profwrite_;
	const double profstart_ = cfg.get("PROF_START", "Output"); profstart=profstart_;
	const double profdaysbetween_ = cfg.get("PROF_DAYS_BETWEEN", "Output"); profdaysbetween=profdaysbetween_;
	const bool tswrite_ = cfg.get("TS_WRITE", "Output"); tswrite=tswrite_;
	const double tsstart_ = cfg.get("TS_START", "Output"); tsstart=tsstart_;
	const double tsdaysbetween_ = cfg.get("TS_DAYS_BETWEEN", "Output"); tsdaysbetween=tsdaysbetween_;
	const bool snow_write_ = cfg.get("SNOW_WRITE", "Output"); snow_write=snow_write_;

	const bool precip_rates_ = cfg.get("PRECIP_RATES", "Output"); precip_rates=precip_rates_;
	const bool avgsum_time_series_ = cfg.get("AVGSUM_TIME_SERIES", "Output"); avgsum_time_series=avgsum_time_series_;
	const bool cumsum_mass_ = cfg.get("CUMSUM_MASS", "Output"); cumsum_mass=cumsum_mass_;
	const double thresh_rain_ = cfg.get("THRESH_RAIN", "SnowpackAdvanced"); thresh_rain=thresh_rain_; //Rain only for air temperatures warmer than threshold (degC)
	const bool advective_heat_ = cfg.get("ADVECTIVE_HEAT", "SnowpackAdvanced"); advective_heat=advective_heat_;
	const bool soil_flux_ = (useSoilLayers || variant == "SEAICE") ? cfg.get("SOIL_FLUX", "Snowpack") : false; soil_flux=soil_flux_;
	...
	//SnowpackIO snowpackio(cfg); // Jafari commented as we already have it in coupling interface class.. 
	//mio::IOManager io(cfg); // Jafari commented as we already have it in coupling interface class..
	...
	//now, let's start!
	printStartInfo(cfg);//, string(argv[0])); // Jafari commented
	...
	//for (size_t i_stn=0; i_stn<vecStationIDs.size(); i_stn++) { Jafari commented
		i_stn=0;// Jafari added, we already have it in coupling interface class.. 
		cout << endl;
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Run on meteo station %s", vecStationIDs[i_stn].c_str());
		run_timer.reset();
		meteoRead_timer.reset();

		//Slope slope(cfg); // Jafari commented, we already have it in coupling interface class
		//Cumsum cumsum(slope.nSlopes); // Jafari commented, we already have it in coupling interface class

		lw_in = Constants::undefined;    // Storage for LWin from flat field energy balance lw_in // Jafari modified, we already declared it in coupling interface class

		// Used to scale wind for blowing and drifting snowpack (from statistical analysis)
		wind_scaling_factor = cfg.get("WIND_SCALING_FACTOR", "SnowpackAdvanced"); // Jafari modified, we already declared it in coupling interface class

		// Control of time window: used for adapting diverging snow depth in operational mode
		time_count_deltaHS = 0.; // Jafari modified, we already declared it in coupling interface class

		// Snowpack data (input/output)
		//ZwischenData sn_Zdata;   // "Memory"-data, required for every operational station // Jafari commented, we already have it in coupling interface class
		vector<SN_SNOWSOIL_DATA> vecSSdata_copy(slope.nSlopes, SN_SNOWSOIL_DATA(/*number_of_solutes*/)); vecSSdata=vecSSdata_copy; // Jafari coppied the created object to that we already have in coupling interface class
		//vector<SnowStation> vecXdata; // Jafari commented, we already have it in coupling interface class
		...
		//CurrentMeteo Mdata(cfg); // Jafari commented, we already have it in coupling interface class
		// To collect surface exchange data for output
		//SurfaceFluxes surfFluxes/*(number_of_solutes)*/; // Jafari commented, we already have it in coupling interface class
		// Boundary condition (fluxes)
		//BoundCond sn_Bdata; // Jafari commented, we already have it in coupling interface class

		current_date=dateBegin; // Jafari modified, we already declared it in coupling interface class		
		...
		if (!read_slope_status) throw; // Jafari changed it to "throw" as "continue" is for a loop ... 
		...
		//vector<ProcessInd> qr_Hdata_ind; //Hazard data Index for t=0...tn  // Jafari commented, we already have it in coupling interface class
		duration = (dateEnd.getJulian() - current_date.getJulian() + 0.5/24)*24*3600; //HACK: why is it computed this way? // Jafari modified, we already declared it in coupling interface class
		...
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg", mio::Date(), "Start simulation for %s on %s",
			vecStationIDs[i_stn].c_str(), current_date.toString(mio::Date::ISO_TZ).c_str());
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "End date specified by user: %s",
		        dateEnd.toString(mio::Date::ISO_TZ).c_str());
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Integration step length: %f min",
...
bool SnowpackInterface::nextStep() // Jafari added...true means continue for the next time step
{	
	// START TIME INTEGRATION LOOP
	do //Jafari changed the do-while condition to false to make it as a simple block of the code to be able to use break command
	{
		...
		if (vecMyMeteo.empty()) {
			if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", current_date, "No forcing data provided for [%s]",
		...
		if (!validMeteoData(vecMyMeteo[i_stn], vecStationIDs[i_stn], variant, enforce_snow_height, advective_heat, soil_flux, slope.nSlopes)) {
			if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", current_date, "No valid data for station %s on [%s]",
			        vecStationIDs[i_stn].c_str(), current_date.toString(mio::Date::ISO).c_str());
			current_date -= calculation_step_length/1440;
			break; //Jafari changed the do-while condition to false to make it as simple block code to be able to use break command here
		}
		...
		for (unsigned int slope_sequence=0; slope_sequence<slope.nSlopes; slope_sequence++) {
			...
			if ((mode == "RESEARCH") && (slope.sector == slope.mainStation)
				...
				if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg", current_date,
			...
			if (tswrite && mn_ctrl.TsDump) {
				...
				if(Foam::Pstream::master()) snowpackio.writeTimeSeries(vecXdata[slope.sector], surfFluxes, Mdata,
			...
			if (profwrite && mn_ctrl.PrDump)
				if(Foam::Pstream::master()) snowpackio.writeProfile(current_date, vecXdata[slope.sector]);
			...
			if (mn_ctrl.XdataDump) {
				...
				if(Foam::Pstream::master()) snowpackio.writeSnowCover(current_date, vecXdata[slope.sector], sn_Zdata, (label_snow)?(2):(1));
				if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg", current_date,
			...
			if (!avgsum_time_series) {
				...
				if (mass_balance) {
					if (massBalanceCheck(vecXdata[slope.sector], surfFluxes, tot_mass_in) == false)
						if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg+", current_date, "Mass error at end of time step!");
				}
			}
		} //end loop on slopes
		computed_one_timestep = true;
	}while(false); //Jafari changed the do-while condition to false to make it as simple block code to be able to use break command
	//} /while ((dateEnd.getJulian() - current_date.getJulian()) > calculation_step_length/(2.*1440)); //Jafari changed the do-while condition to false to make it as a simple block of the code to be able to use break command
	//end loop on timesteps..
	
	bool conditionTocontinueToEndDate=(dateEnd.getJulian() - current_date.getJulian()) > calculation_step_length/(2.*1440); // Jafari added
	if(!conditionTocontinueToEndDate) //Jafari added, the SNOWPACK simulation is completed from dateStart to dateEnd 
	{
		...
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Total time to read meteo data : %lf s",
				meteoRead_timer.getElapsed());
		if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg-", mio::Date(), "Runtime for station %s: %lf s",
				vecStationIDs[i_stn].c_str(), run_timer.getElapsed());

		time_t nowEND=time(NULL);
		if(Foam::Pstream::master()) cout << endl;
		if(Foam::Pstream::master()) cout << "[i] []                 STARTED  running SLF " << mode << " Snowpack Model on " << ctime(&nowSRT);
		if (mode == "OPERATIONAL"){
			if(Foam::Pstream::master()) cout << "                       ===========================================================================" << endl;
		} else {
			if(Foam::Pstream::master()) cout << "                       ========================================================================" << endl;
		}
		if(Foam::Pstream::master()) cout << "                       FINISHED running SLF " << mode << " Snowpack Model on " << ctime(&nowEND) << endl;
		return false; // Jafari added
	}
	else
	{
		return true; // Jafari added
	}
}
