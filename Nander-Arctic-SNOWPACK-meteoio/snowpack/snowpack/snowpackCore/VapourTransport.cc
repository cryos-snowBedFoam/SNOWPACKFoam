...
VapourTransport::VapourTransport(const SnowpackConfig& cfg)
               : WaterTransport(cfg), RichardsEquationSolver1d(cfg, false), variant(),
                 ...
                 useSoilLayers(false), water_layer(false), vapour_transport_model("NONE"),enable_vapour_transport(false),
...
{
	// compSurfaceSublimation will be done here in any case with/without vapor transport
