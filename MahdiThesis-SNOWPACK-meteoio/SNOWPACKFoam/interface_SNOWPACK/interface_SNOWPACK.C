...
SnowpackInterface::SnowpackInterface()
                   : /*cfg(cfgfile),snowpackio(cfg),io(cfg), vecSSdata(), vecXdata(false,false,false), Mdata(cfg),
                     surfFluxes(),sn_Bdata(),slope(cfg),cumsum(slope.nSlopes),current_date(dateBegin), hazard(cfg,0.)*/
                    addSpecialKeyToCfg(),io(cfg),snowpackio(cfg),/*vecSSdata(slope.nSlopes),vecXdata(slope.nSlopes),*/Mdata(cfg),
                    surfFluxes(),sn_Bdata(),slope(cfg),cumsum(slope.nSlopes),current_date(dateBegin)//,hazard(cfg,1.e1)
{
	init_sn();
}
...
//#endif
	...
	std::string end_date_str = cfg.get("ENDDATE", "Input"); 
	   //std::cout << "Starting date is:\t" <<  begin_date_str << std::endl ;
	   //std::cout << "End date is:\t" <<  end_date_str << std::endl ;

	//parseCmdLine(argc, argv, begin_date_str, end_date_str); // Jafari deleted

	prn_check = false; // Jafari modified, we already declared it in coupling interface class
	...
	const bool snowPrep_ = cfg.get("SNOW_PREPARATION", "SnowpackAdvanced"); snowPrep=snowPrep_;
	...
	//for (size_t i_stn=0; i_stn<vecStationIDs.size(); i_stn++) { Jafari commented
		...
		if (!read_slope_status) throw; //something went wrong, move to the next station
...
{	
	...
	do //Jafari changed the do-while condition to false to make it as simple block code to be able to use break command
	{
		...
		for (unsigned int slope_sequence=0; slope_sequence<slope.nSlopes; slope_sequence++) {
			...
			if (slope.sector == slope.mainStation) { // main station only (usually flat field)
				...
				if (mode == "OPERATIONAL") {
					...
					if (enforce_snow_height && (fabs(time_count_deltaHS) > (1. - 0.05 * M_TO_D(calculation_step_length)))) {
						...
						if (prn_check) {
							if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg+", Mdata.date,
			...
			if (mn_ctrl.XdataDump) {
				...
				if(Foam::Pstream::master()) snowpackio.writeSnowCover(current_date, vecXdata[slope.sector], sn_Zdata, true);
				if(Foam::Pstream::master()) prn_msg(__FILE__, __LINE__, "msg", current_date,
	...
	//} /while ((dateEnd.getJulian() - current_date.getJulian()) > calculation_step_length/(2.*1440)); //Jafari changed the do-while condition to false to make it as simple block code to be able to use break command
