                                       ...
                                       SnowStation& Xdata, SurfaceFluxes& Sdata, const double& surfaceVaporPressure)
{
	...
	// (1) if we do it like the first diffusive paper, we just use ql in VapourTransport_OF_to_SN not in waterTransport i.e. the case vapour_transport_model =="convection_OpenFOAM"
	// (2) if we use ql first in waterTransport (which is the case now as i.e. vapour_transport_model =="diffusion_SNOWPACK"), then the leftover is used here in vaporTransport ....	
	compSurfaceSublimation(Mdata, ql, Xdata, Sdata);
	ql=0;
