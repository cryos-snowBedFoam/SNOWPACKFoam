...
#include <snowpack/snowpackCore/VapourTransport_OF_to_SN.h> // Jafari added
            ...
            temp_index_degree_day(0.), temp_index_swr_factor(0.), forestfloor_alb(false), soil_evaporation(EVAP_RELATIVE_HUMIDITY)
{
	...
	//std::cout << "HEIGHT_NEW_ELEM/MINIMUM_L_ELEMENT " << height_new_elem << ' ' << minimum_l_element << endl;
...
bool Snowpack::sn_ElementKtMatrix(ElementData &Edata, double dt, const double dvdz, double T0[ N_OF_INCIDENCES ], double Se[ N_OF_INCIDENCES ][ N_OF_INCIDENCES ], double Fe[ N_OF_INCIDENCES ], const double VaporEnhance)
{
	...
	///the calculation Keff is based on SNOWPACK
...
/*
	/// J added: the calculation Keff for snow is based on Hansen and for the soil is based on SNOWPACK
	double Keff;    // the effective thermal conductivity
	if (Edata.theta[SOIL] > 0.0) {
		Keff = SnLaws::compSoilThermalConductivity(Edata, dvdz);
		Keff *= VaporEnhance; // for the soil VaporEnhance seems to be 1
	} 
	else {
		Keff = SnLaws::compSnowThermalConductivity_Hansen(Edata);
	}	
	Edata.k[TEMPERATURE] = Keff;
	const double k = Keff/Edata.L;   //Conductivity. Divide by the length to save from doing it during the matrix operations
*/	
	...
	if (snow_fall && snowed_in) {
		...
		if ((delta_cH >= height_new_elem * cos_sl) || (Xdata.hn > 0.) || add_element) {
			...
			if (nHoarE > 0) {
				...
				// J added updating parameters for SNOWPACK-OpenFOAM
				///EMS[nOldE-1].SN_to_OF_deltaTheta_ice-=EMS[nOldE-1].theta[ICE]; // after het---we do not need as new element is initialized zero and we do not use this one
				///EMS[nOldE-1].SN_to_OF_deltaTheta_water-=EMS[nOldE-1].theta[WATER]; // after het----we do not need as new element is initialized zero and we do not use this one								
                                ...
                                BoundCond& Bdata, SurfaceFluxes& Sdata)
{
	...
	VapourTransport vapourtransport(cfg); //Jafari added: this is not needed any more as we have  and was commented
	VapourTransport_OF_to_SN VapourTransport_OF_to_SN(cfg);
...
/////////////////////////OF-SN coupleing///////////////////////////
		// (1) Jafari moved this to the front ....this is because we need to use this snow density change due to OF calculation first and then go for 
		// other process...this is different when we have just SN diffusive vapor transport...we need to update ql here..this is more accurate
		// (2) but maybe we can put vaportransport like the diffuisve paper at the end if some erros happen
...
/////////////////////////OF-SN coupleing///////////////////////////
		for(size_t i=Xdata.SoilNode; i<Xdata.getNumberOfElements(); i++)
		{
			Xdata.Edata[i].SN_to_OF_deltaTheta_ice+=Xdata.Edata[i].theta[ICE];
			Xdata.Edata[i].SN_to_OF_deltaTheta_water+=Xdata.Edata[i].theta[WATER];
			//std::cout << "delta ice/water---ice/water " << i << ' ' << Xdata.Edata[i].SN_to_OF_deltaTheta_ice << ' ' << Xdata.Edata[i].SN_to_OF_deltaTheta_water << "---- " << Xdata.Edata[i].theta[ICE] << ' ' << Xdata.Edata[i].theta[WATER] << "----nE= " << Xdata.getNumberOfElements() << std::endl;
		}
/////////////////////////OF-SN coupleing///////////////////////////	
}
